...
function setupIPC() {
  ...
  ipcMain.handle('video-time-update', (_event, time: number, duration: number) => {
  ...
  ipcMain.handle('get-bible-chapter', (_event, bookId: string, chapter: number) => {
    return getBibleChapter(bookId, chapter)
  })

  ipcMain.handle('load-bible-verses', (_event, bookId: string, bookName: string, chapter: number, startVerse: number, endVerse?: number) => {
    const verses = getBibleVerses(bookId, chapter, startVerse, endVerse)
    if (verses.length > 0) {
      const { title, slides } = formatBibleVersesForDisplay(bookName, chapter, verses)
      stateManager.loadText(title, slides.join('\n\n'))
    }
  })
}
